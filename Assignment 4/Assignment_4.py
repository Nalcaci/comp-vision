...
def train_model(model, train_loader, val_loader, criterion, optimizer, num_epochs, device):
    ...
    for epoch in range(num_epochs):
        ...
        running_wh_loss = 0.0
        running_conf_obj_loss = 0.0
        running_conf_noobj_loss = 0.0
        ...
        for images, targets in train_loader:
            ...
            total_loss, loss_components = criterion(outputs, targets, return_components=True)
            total_loss.backward()
            optimizer.step()

            batch_size = images.size(0)
            running_loss_total += total_loss.item() * batch_size
            running_coord_loss += loss_components['coord_loss'].item() * batch_size
            running_wh_loss += loss_components['wh_loss'].item() * batch_size
            running_conf_obj_loss += loss_components['conf_loss_obj'].item() * batch_size
            running_conf_noobj_loss += loss_components['conf_loss_noobj'].item() * batch_size
            running_class_loss += loss_components['class_loss'].item() * batch_size

        epoch_train_loss = running_loss_total / len(train_loader.dataset)
        epoch_coord_loss = running_coord_loss / len(train_loader.dataset)
        epoch_wh_loss = running_wh_loss / len(train_loader.dataset)
        epoch_conf_obj_loss = running_conf_obj_loss / len(train_loader.dataset)
        ...
        history['train_total_loss'].append(epoch_train_loss)
        history['train_coord_loss'].append(epoch_coord_loss)
        history['train_wh_loss'].append(epoch_wh_loss)
        history['train_conf_obj_loss'].append(epoch_conf_obj_loss)
        history['train_conf_noobj_loss'].append(epoch_conf_noobj_loss)
        history['train_class_loss'].append(epoch_class_loss)
        history['val_total_loss'].append(epoch_val_loss)

        print(f"Epoch {epoch+1}: Train Loss = {epoch_train_loss:.4f} (Coord {epoch_coord_loss:.4f}, WH {epoch_wh_loss:.4f}, "
...
def main():
    ...
    plt.plot(epochs, history['train_conf_obj_loss'], label='Conf Obj Loss')
    plt.plot(epochs, history['train_conf_noobj_loss'], label='Conf NoObj Loss')
    plt.plot(epochs, history['train_class_loss'], label='Class Loss')
    plt.xlabel("Epoch")
    plt.ylabel("Loss")
